...
import { Plus, Building2, Link2, Trash2, AlertTriangle, Loader2, RefreshCw, XCircle, Info } from "lucide-react";
...
export default function ClientsAccountsPage() {
  ...
  const [clientPressCount, setClientPressCount] = useState<Record<string, number>>({});
  ...
  function loadPressCounts() {
    api.get("/analytics/client-summary").then((r) => {
      const counts: Record<string, number> = {};
      (r.data || []).forEach((s: any) => { counts[s.id] = s.press_source_count ?? 0; });
      setClientPressCount(counts);
    }).catch(() => {});
  }
      ...
      (r.data || []).forEach((c: any) =>
        api.get("/profiles", { params: { client_id: c.id } }).then((p) => {
          ...
          (p.data || []).forEach((pr: any) => {
            loadSummary(pr.id);
            // Seed sync state for active profiles on load
  ...
  useEffect(() => { load(); loadPressCounts(); }, []); // eslint-disable-line react-hooks/exhaustive-deps

  // Check whether an Instagram session is configured (controls whether real data can be scraped)
  useEffect(() => {
    api.get("/admin/instagram-session")
      .then((r) => setIgConfigured(!!r.data?.configured))
      .catch(() => setIgConfigured(null));
  }, []);
  ...
  async function delAccount(id: string) {
    try { await api.delete(`/profiles/${id}`); } catch {}
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Honest notice when any Instagram account is showing estimated (non-live) data */}
      {Object.values(profiles).flat().some((p: any) => p.platform === "instagram" && p.meta?.data_source === "estimated") && (
        <div className="flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-800 p-3">
          ...
          <div className="text-sm text-amber-800 dark:text-amber-300">
            <strong>Some Instagram numbers are estimated, not live.</strong> Instagram blocks this server's IP from
            its data endpoints — even with a valid session cookie, profile/post lookups are rate-limited (HTTP 429),
            so those accounts fall back to representative estimates (clearly tagged <em>Estimated</em> below).
            The reliable way to get <strong>real live posts &amp; comments</strong> is the{" "}
            <strong>browser extension</strong> (it runs from your own browser/IP). Ask to set it up, or use{" "}
            <Link href="/import" className="font-semibold underline hover:no-underline">Import Data</Link> for sheet-based real data.
          </div>
        </div>
      )}
        ...
        <Card key={c.id}>
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 shrink-0 flex-wrap">
              {(clientPressCount[c.id] ?? 0) > 0 && (
                <a
                  href={`/api/v1/press-sources?client_id=${c.id}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  title={`${clientPressCount[c.id]} press channels linked to ${c.name} — click to view`}
                  className="flex items-center gap-1.5 rounded-lg border border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-900/20 px-3 py-1.5 text-xs font-medium text-purple-700 dark:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-900/40 transition-colors">
                  📰 {clientPressCount[c.id]} press channel{clientPressCount[c.id] !== 1 ? "s" : ""}
                </a>
              )}
              <button
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {(profiles[c.id] || []).map((p) => {
              ...
              const isActive = ACTIVE_STATUSES.has(status);
              // Instagram only shows real (bookmarklet) data; anything else → prompt for live sync
              const needsLive = p.platform === "instagram" && p.meta?.data_source !== "live";
              return (
                <div key={p.id} className={`rounded-xl border p-3 space-y-2 transition-colors ${
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      {/* Honest data-source label: live scrape vs estimated (IP-blocked) */}
                      ...
                      {!isActive && p.meta?.data_source === "estimated" && (
                        <span className="mt-1 inline-flex items-center gap-1 rounded-full bg-amber-500/15 text-amber-600 px-1.5 py-0.5 text-[10px] font-semibold"
                          title="Instagram blocked the server IP, so these are representative estimates, not live numbers.">
                          ● Estimated
                        </span>
                      )}
                    </div>
                  ...
                  {!isActive && (
                    needsLive ? (
                      <div className="rounded-lg border border-amber-300 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-800 p-2.5 space-y-1.5">
                        <p className="text-[11px] text-amber-800 dark:text-amber-300 leading-snug">
                          {p.meta?.data_source === "estimated"
                            ? "These numbers are estimated — not live."
                            : "No live data yet."}{" "}
                          Instagram blocks the server, so pull the real last-90-days posts from your own browser.
                        </p>
                        <Link href="/admin/sync"
                          className="flex items-center justify-center gap-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white py-1.5 text-xs font-semibold transition-colors">
                          📌 Get live data
                        </Link>
                      </div>
                    ) : (
                  ...
                  <div className="flex gap-2 mt-1">
                    {p.platform === "youtube" && (
                      <button
                        ...
                        title="Pull the latest 90 days of videos & comments again"
                    ...
                    {p.platform === "instagram" && p.meta?.data_source === "live" && (
                      <Link href="/admin/sync"
                        title="Re-run the Instagram Sync bookmarklet to refresh live data"
                        className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-blue-200 dark:border-blue-800 py-1.5 text-xs font-medium text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
        ...
        <div className="space-y-3">
          ...
          {acc.platform === "instagram" && (
            <p className="rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
              Instagram blocks our server, so connecting only registers the account. To pull the <strong>real last-90-days posts &amp; comments</strong>, open <strong>Sync Engine</strong> and use the one-click <strong>Instagram Sync bookmarklet</strong> from your own browser.
            </p>
          )}
