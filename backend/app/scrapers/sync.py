    ...
    try:
        ...
        if platform == "youtube":
            ...
            try:
                ...
                logger.warning("SYNC job=%s: Facebook scrape failed: %s", job_id, str(fb_err)[:120])
                _wipe_profile_posts(db, profile.id)
                _status(job_id, "done", error=None)
                _persist(db, profile, stage="done", data_source="pending",
                         posts_done=0, total_posts=0, comments_done=0)
                return

        elif platform == "instagram":
            handle_or_url = profile.profile_url or profile.handle or ""
            # Try Instagram Graph API first (uses same Facebook token — free, reliable)
            import os as _ig_os
            from app.scrapers.facebook_adapter import load_fb_config as _load_fb
            _fb_tok = (_ig_os.environ.get("FACEBOOK_ACCESS_TOKEN", "").strip()
                       or _load_fb().get("access_token", "").strip())
            if _fb_tok:
                try:
                    from app.scrapers.instagram_graph_adapter import InstagramGraphAdapter
                    _ig_graph = InstagramGraphAdapter()
                    _graph_posts = await _ig_graph.fetch_profile_posts(
                        handle_or_url, max_posts=max_posts, days=days, token=_fb_tok
                    )
                    if _graph_posts:
                        posts_fetched = _graph_posts
                        adapter = _ig_graph
                        data_source = "live"
                        logger.info("SYNC job=%s: IG Graph API fetched %d posts", job_id, len(posts_fetched))
                except Exception as _ig_graph_err:
                    logger.warning("SYNC job=%s: IG Graph API failed: %s", job_id, str(_ig_graph_err)[:120])

            # Fall back to instaloader/session-cookie scraping if Graph API not available or failed
            if not posts_fetched:
                ...
                try:
                    ...
                    if meta_info:
                        ...
                        db.commit()
                    logger.info("SYNC job=%s: scraped %d real posts via instaloader", job_id, len(posts_fetched))
                except Exception as scrape_err:
                    logger.warning("SYNC job=%s: server can't scrape IG (%s) — awaiting bookmarklet",
                                   job_id, str(scrape_err)[:120])
                    cur_source = (profile.meta or {}).get("data_source")
                    if cur_source == "live":
                        _status(job_id, "done", error=None)
                        _persist(db, profile, stage="done", data_source="live")
                    else:
                        _wipe_profile_posts(db, profile.id)
                        _status(job_id, "done", error=None)
                        _persist(db, profile, stage="done", data_source="pending",
                                 posts_done=0, total_posts=0, comments_done=0)
                    logger.info("SYNC job=%s: IG profile marked '%s' (no fake data)",
                                job_id, "live" if cur_source == "live" else "pending")
                    return

        else:
            _status(job_id, "done (no adapter)", error=f"Platform '{platform}' not auto-synced yet.")
            _persist(db, profile, stage="error", error=f"Platform '{platform}' not auto-synced yet.")
            return
        ...
        for i, np in enumerate(posts_fetched):
            ...
            if not existing:
                ...
                post.metrics = {**(post.metrics or {}), "real_comment_total": real_comment_count}
                # Backfill permalink if missing (fixes posts synced before this patch)
                if not post.permalink and np.url:
                    post.permalink = np.url
                if not post.url and np.url:
                    post.url = np.url
...
def _generate_fallback_posts(profile: SocialProfile, days: int, max_posts: int):
    """Generate synthetic NormalizedPost objects for a profile when real scraping fails."""
    from app.scrapers.base_adapter import NormalizedPost
    import random
    handle = (profile.handle or "unknown").lstrip("@")
    posts = []
    now = datetime.now(timezone.utc)
    n = min(max_posts, days)  # ~1 post/day
    seed = int(hashlib.md5(handle.encode()).hexdigest()[:8], 16)
    rng = random.Random(seed)
    captions = [
        "Addressing key issues facing our citizens today",
        "Working hard for the people every single day",
        "Important update on development projects",
        "Meeting with citizens and listening to their concerns",
        "Progress update on infrastructure development",
        "Proud to serve and represent the people",
        "Strengthening our commitment to public welfare",
        "New initiative launched for community development",
        "Grateful for the trust and support of the people",
        "Continuing our work for a better tomorrow",
    ]
    for i in range(n):
        days_ago = int((i / max(n - 1, 1)) * (days - 1))
        post_date = now - timedelta(days=days_ago, hours=rng.randint(0, 12))
        comment_count = rng.randint(80, 500)
        posts.append(NormalizedPost(
            external_id=f"fallback_{handle}_{i}_{seed}",
            source_kind=profile.platform or "instagram",
            content=captions[i % len(captions)],
            author=handle,
            url=f"https://www.instagram.com/{handle}/",
            published_at=post_date,
            metrics={"likes": rng.randint(500, 10000), "comments": comment_count,
