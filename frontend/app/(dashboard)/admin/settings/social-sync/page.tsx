  ...
  const startSync = async () => {
    ...
    try {
      const r = await fetch(`${API}/social-sync/profiles/${profile.id}/sync`, {
        ...
        body: JSON.stringify({ max_posts: 30, days: 90 }),
      });
      const data = await r.json();
      if (data.job_id) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <div className="mb-6 rounded-2xl border border-blue-200 bg-blue-50 dark:border-blue-900 dark:bg-blue-950/30 px-4 py-3 text-sm text-blue-700 dark:text-blue-300">
        <strong>How it works:</strong> Social Sync fetches real posts and top comments directly from public social media accounts. YouTube video titles are fetched via yt-dlp (no API key needed); view counts require a <strong>YouTube Data API key</strong> (free — set it in Sync Settings). Facebook &amp; Instagram fall back to AI-estimated data when the server IP is rate-limited. Click <strong>Sync Now</strong> on any account to refresh.
      </div>
